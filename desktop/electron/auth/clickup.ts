...
export async function logoutClickUp() {
  ...
  // await window.webContents.loadURL(`${FRONTEND_URL}/api/backend/v1/clickup/unlink${webhookId ? "/" + webhookId : ""}`, {
  //   userAgent,
  // });
  //
  // for (let i = 0; i < 30; i++) {
...
export function initializeClickUpAuthHandler() {
  ...
  logoutClickUpBridge.handle(() => logoutClickUp());
}
