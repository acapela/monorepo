...
import { IndexValueInput, IndexableKey } from "./queryIndex";
import { EntityStore } from "./store";

type FindObjectPartInput<T, C> = Partial<{
  [key in keyof T]: key extends IndexableKey<Entity<T, C>> ? IndexValueInput<T[key]> : never;
}>;

export type FindObjectInput<T, C> = FindObjectPartInput<T, C> & {
  $or?: FindObjectPartInput<T, C>[];
};
