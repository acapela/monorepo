...
export function pluralize(strings: TemplateStringsArray, ...interpolations: Array<PluralInterpolation>) {
  const numberInterpolations = interpolations.filter(
    (interpolation): interpolation is number => typeof interpolation === "number"
  );
  ...
  for (const [index, part] of strings.entries()) {
    ...
    const interpolation: PluralInterpolation | null = interpolations[index] ?? null;
    ...
    if (Array.isArray(interpolation)) {
      if (interpolation.length === 1) {
        results.push(autoPluralize(interpolation[0], count));
      } else {
...
export function autoPluralize(word: string, amount?: number): string {
